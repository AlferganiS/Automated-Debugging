...
from debuggingbook.Repairer import StatementMutator, Repairer, ConditionVisitor, all_conditions


def comparator_mutation_candidates(node: ast.Compare) -> Set[ast.Compare]:
    nodes_set = set()
    if isinstance(node.ops[0], ast.LtE):
        compare_node = ast.Compare(left=node.left, ops=[ast.Lt()], comparators=node.comparators)
        nodes_set.add(compare_node)
        compare_node = ast.Compare(left=node.left, ops=[ast.GtE()], comparators=node.comparators)
        nodes_set.add(compare_node)
        return nodes_set
    elif isinstance(node.ops[0], ast.Lt):
        compare_node = ast.Compare(left=node.left, ops=[ast.LtE()], comparators=node.comparators)
        nodes_set.add(compare_node)
        compare_node = ast.Compare(left=node.left, ops=[ast.Gt()], comparators=node.comparators)
        nodes_set.add(compare_node)
        return nodes_set
...
def is_atomic(cond: ast.expr) -> bool:
    match cond:
        case ast.BoolOp():
            return False
        case ast.UnaryOp(op, _):
            return not isinstance(op, ast.Not)
        case _:
            return True

def collect_atomic_conditions(tree: ast.AST) -> List[ast.expr]:
    return [t for t in all_conditions(tree) if is_atomic(t)]
...
class ConditionGenerator:
    ...
    def __init__(self, tree: ast.AST) -> None:
        self.atomic_conditions: List[ast.expr] = collect_atomic_conditions(tree)

    def choose_term(self) -> ast.expr:
        # <term> ::= <atomic> | comparator mutation candidates of an <atomic>
        atomic = random.choice(self.atomic_conditions)
        
        if isinstance(atomic, ast.Compare):
            candidates = list(comparator_mutation_candidates(atomic))
            if len(candidates) > 0:
                return random.choice([atomic] + candidates)
        
        return atomic


    def choose_simple(self) -> ast.expr:
        # <simple> ::= <term> | not <term>
        term = self.choose_term()
        
        if random.choice([True, False]):
            return ast.UnaryOp(op=ast.Not(), operand=term)
    ...
    def sample(self) -> ast.expr:
        ...
        match random.choice(['and', 'or', '']):
            case 'and':
                return ast.BoolOp(op=ast.And(), values=[self.choose_simple(), self.choose_simple()])
            case 'or':
                return ast.BoolOp(op=ast.Or(), values=[self.choose_simple(), self.choose_simple()])
            case _:
                return self.choose_simple()
...
class ConditionMutator(StatementMutator):
    ...
    def swap(self, node: ast.AST) -> ast.AST:
        ...
        cond = self.condition_generator.sample()
        new_test = None

        match self.choose_bool_op():
            case 'set':
                new_test = cond
            case 'not':
            ...
            case _:
                raise ValueError("Unknown boolean op")

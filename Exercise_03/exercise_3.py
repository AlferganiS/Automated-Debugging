...
class Station:
    ...
    def repOK(self):
        ...
        for track in self.tracks:
            assert self is track.start or self is track.end 
...
class TrainNetwork:
    ...
    def repOK(self):
        ...
        for train_1 in self.trains:
            for train_2 in self.trains:
                if train_1 is not train_2:
                    assert train_1.train_id != train_2.train_id
        for track_1 in self.tracks:
            if track_1.train is not None:
                for track_2 in track_1.start.tracks:
                    if track_1 is not track_2:
                        assert track_2.train is None
                for track_2 in track_1.end.tracks:
                    if track_1 is not track_2:
                        assert track_2.train is None 
    ...
    def move_train(self, train: Train, from_track: Track, to_track: Track):
        self.repOK()
        # TODO: add preconditions
        assert train in self.trains, 'Train is not in Network'
        assert from_track in self.tracks, 'Track is not part of the network'
        assert to_track in self.tracks, 'Track is not part of the network'
        assert train is from_track.train
        assert to_track in from_track.start.tracks or to_track in from_track.end.tracks     
        assert to_track.train is None, 'Track is not empty'

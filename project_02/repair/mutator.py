...
class Marker(ast.NodeTransformer):
            ...
            if isinstance(old_value, list):
                ...
                for value in old_value:
                    if isinstance(value, ast.AST):
                        self.loop_level += self.check_loop_enter(value)
                        self.is_first_stmt = self.check_first_stmt(value, old_value)
                        value = self.visit(value)
                        self.loop_level -= self.check_loop_exit(value)
                        if value is None:
                            continue
                ...
                self.loop_level += self.check_loop_enter(old_value)
                new_node = self.visit(old_value)
                self.loop_level -= self.check_loop_exit(new_node)
                if new_node is None:
                    delattr(node, field)
                else:
    ...
    def check_loop_enter(self, node: ast.AST) -> int:
        """
    ...
    def check_loop_exit(self, node: ast.AST) -> int:
        """
        Checks if the given node is a loop node and returns 1 if it is, 0 otherwise.
        """
        if isinstance(node, ast.While) or isinstance(node, ast.For):
    ...
    def check_first_stmt(self, value, old_values: list) -> bool:
        if value == old_values[0]:
            return True
        else:
            return False

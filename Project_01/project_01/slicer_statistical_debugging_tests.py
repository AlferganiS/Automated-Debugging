...
class DebuggerRankingTests(DebuggerTests):
    ...
    def test_middle(self):
        ...
        try:
            self.assertIn(('middle', 6), debugger.rank()[:3])
        except AssertionError:
            self.assertIn(('middle', 15), debugger.rank()[:3])
    ...
    def test_remove_html_markup(self):
        ...
        try:
            ...
            self.assertIn(('remove_html_markup', 20), debugger.rank()[:3])
    ...
    def test_sqrt(self):
        ...
        try:
            ...
            self.assertIn(('square_root', 8), debugger.rank()[:3])

    def test_bf(self):
        debugger = self.bf_tester(coverage=True)
        try:
            self.assertIn(('interpret', 14), debugger.rank()[:5])
            self.assertIn(('interpret', 15), debugger.rank()[:5])
        except AssertionError:
            self.assertIn(('interpret', 16), debugger.rank()[:5])
            self.assertIn(('interpret', 17), debugger.rank()[:5])

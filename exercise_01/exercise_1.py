...
def convert_to_rgb(hex_str: str) -> Tuple[int, int, int]:
    assert check_len(hex_str), "Invalid lenght"
...
def traceit(frame, event, arg):
    ...
    return traceit
def convert_to_rgb_traced(hex_str: str) -> Tuple[int, int, int]:
    assert check_len(hex_str)
    sys.settrace(traceit)
    ret = convert_to_rgb(hex_str)
    sys.settrace(None)
    return ret
print(convert_to_rgb("1919F9"))
